...
class BoxDiffPipeline(StableDiffusionGLIGENPipeline):
    ...
    def _encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            concat_embeds = []
            for i in range(0, text_input_ids.shape[-1], self.tokenizer.model_max_length):
                concat_embeds.append(
                    self.text_encoder(text_input_ids[:, i: i + self.tokenizer.model_max_length], attention_mask=attention_mask)[0]
                  )
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            uncond_input_idx = uncond_input.input_ids.to("cuda")
            ...
            concat_embeds = []
            for i in range(0, uncond_input_idx.shape[-1], self.tokenizer.model_max_length):
                concat_embeds.append(
                    self.text_encoder(uncond_input_idx[:, i: i + self.tokenizer.model_max_length], attention_mask=attention_mask)[0]
                  )

            negative_prompt_embeds = torch.cat(concat_embeds, dim=1)
